...
public class UNXMLHandler extends DefaultHandler {
	 /* A buffer for each XML element */
	  ...
	  public UNXMLHandler(File xmlFile) 
	  ...
	  public void startElement(String namespaceURI, String localName,
	    ...
	    String eName = localName;
	     ...
	     if(eName=="p"){
	    	 numSent=Integer.parseInt(attrs.getValue("n"));
	     }
	  ...
	  public void characters(char[] text, int start, int length)
	    throws SAXException {
	  	if(elemStack.peek().equalsIgnoreCase("s")){
	  		if(numSent ==1) titleBuffer.append(text, start, length);
